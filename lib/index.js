...
function jslinter(update) {
    var jslintProgram = update
        ? helpers.download(requestObject, jslintFilePath)
        : helpers.readFile(jslintFilePath)
            .catch(function () {
                console.log("failed opening local jslint.js trying to download...");
                return helpers.download(requestObject, jslintFilePath);
            });

    return jslintProgram
        .then(function (jsLintProgram) {
            var jslint = compileJSLint(jsLintProgram.replace("export default function jslint(", "function jslint("));
            var result = jslint(jsLintProgram);
            if (!result.ok) {
                throw new Error("JSLint did not approve itself! This should not happen!");
            }
            return {
                jslint: jslint,
